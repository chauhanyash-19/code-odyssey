...
IMPORTANT VALIDATION NOTE (Real-World Test):
PDI threshold (0.6) was originally tuned on synthetic sine-wave signals. Validation against real gTTS-generated speech and real human recordings showed massive overlap and inversion: human samples scored ~0.89 PDI, while gTTS scored ~0.73 PDI. Both were falsely classified as synthetic under the 0.6 threshold. This indicates the current bispectrum approach needs further tuning or additional features (formant stability, ensemble scoring per Phase 4) before being presented as a standalone reliable deepfake detector. It functions as a genuine research-stage signal, not a production-validated classifier.
